...
public class EventosController {
    ...
    private EventosDto toDto(Eventos u) {
    ...
    public ResponseEntity<List<EventosDto>> lista(){
        ...
        return ResponseEntity.ok(
                eventos.stream().map(this::toDto).collect(Collectors.toList()));
    }
    ...
    public ResponseEntity<EventosDto> save(@RequestBody EventosDto eventosDto){
        Eventos u = Eventos
                ...
                // Si no viene fecha desde el cliente, la pone el servidor
                .fecha(eventosDto.getFecha() != null ? eventosDto.getFecha() : LocalDateTime.now())
                .build();
